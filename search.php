  ...
  <body>
    ...
    <div id="container">
        <?php 
            ...
            $sql = "SELECT * FROM `bloggerpost`.`blog` WHERE `blog`.`name` LIKE '$search%' OR `blog`.`tags` LIKE '%$search%' OR `blog`.`author_name` LIKE '%$search%' ORDER BY LENGTH(`blog`.`name`) ,`blog`.`name` ASC;";
            $result = $con->query($sql);
